...
use super::ast::{
    AlterTableAction, ColumnDef, CreateIndexStatement, SelectStatement, TableConstraint,
};
...
pub fn logical_to_physical_plan(
    ...
    match plan {
        ...
        LogicalPlan::CreateView { view_name, query } => {
            Ok(PhysicalPlan::CreateView { view_name, query })
        }
        LogicalPlan::CreateSchema { schema_name } => Ok(PhysicalPlan::CreateSchema { schema_name }),
        LogicalPlan::DropTable { table_name } => Ok(PhysicalPlan::DropTable { table_name }),
        LogicalPlan::DropView { view_name } => Ok(PhysicalPlan::DropView { view_name }),
        LogicalPlan::Insert {
            table_name,
            columns,
            source,
            ...
            let source_column_names =
                if let LogicalPlan::Projection { expressions, .. } = current_source {
                    expressions
                        .iter()
        ...
        LogicalPlan::WorkingTableScan { cte_name, alias } => {
            Ok(PhysicalPlan::WorkingTableScan { cte_name, alias })
        }
        LogicalPlan::GraphMatch {
            query,
            returns,
            alias,
        } => Ok(PhysicalPlan::GraphMatch {
            query,
            returns,
            alias,
        }),
