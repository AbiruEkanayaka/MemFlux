...
def test_recovery(sock, reader, ffi_path=None):
    def send(parts):
    ...
    if ffi_path is None:
        ...
        print("[INFO] This test will simulate a restart by closing and reopening the FFI database.")
    ...
    while True:
        sock, reader = create_connection(retry=True, ffi_path=ffi_path)
        if sock:
