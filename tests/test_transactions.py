...
class ConnHelper:
    """A helper class to wrap a connection and its commands."""
    def __init__(self, sock, reader, name):
        self.sock = sock
        self.reader = reader
        self.name = name

    def send(self, parts):
        resp, *_ = send_resp_command(self.sock, self.reader, parts)
        return resp.strip()
...
def test_transactions(sock, reader, ffi_path=None):
    if ffi_path:
        ...
        print("[INFO] Skipping transaction tests for FFI mode as it is not yet supported.")
        return

    c1 = ConnHelper(sock, reader, "C1")
    
    conn2, reader2 = None, None
    try:
        ...
        # Verify changes are visible on the second connection after COMMIT
        ...
        # Verify changes are NOT visible after ROLLBACK (C2)
        print(f"[INFO] Verifying changes are NOT visible on {c2.name} after ROLLBACK...")
